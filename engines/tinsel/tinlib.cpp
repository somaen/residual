 ...
 * $Id: tinlib.cpp 40884 2009-05-25 11:18:29Z Hkz $
 *
...
enum MASTER_LIB_CODES {
	...
	WALKTAG, WALKXPOS, WALKYPOS, WHICHCD, WHICHINVENTORY, ZZZZZZ, NOFUNCTION, PLAYSEQUENCE,
	DEC3D, SETHYPERLINK, DECINVMAIN, ADDNOTEBOOK, ADDCONV, ADDINV8, ADDINV3, ADDINV4, ADDINV7, NOIREMPTY,
	HIGHEST_LIBCODE, UNKNOWN_CODE, LIB_STUB1, LIB_STUB2, LIB_STUB3
};
...
const MASTER_LIB_CODES NOIR_CODES[] = {
	NOFUNCTION, ACTORBRIGHTNESS, ACTORDIRECTION, ACTORPRIORITY,
	ACTORREF, ACTORRGB, ACTORXPOS, ACTORYPOS, ADDNOTEBOOK, ADDCONV, NOFUNCTION, ADDINV8,
	ADDINV1, ADDINV2, ADDINV7, ADDINV4, ADDINV3,
	ADDTOPIC, BACKGROUND, BLOCKING, UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE, 
	CALLSCENE, UNKNOWN_CODE, CAMERA, CDCHANGESCENE,
	CDDOCHANGE, CDENDACTOR, CDLOAD, CDPLAY, UNKNOWN_CODE, CLEARHOOKSCENE, CLOSEINVENTORY, UNKNOWN_CODE,
	CONTROL, CONVERSATION, UNKNOWN_CODE, CURSOR, CURSORXPOS, CURSORYPOS, DECINVMAIN, DECINV2,
	DECLARELANGUAGE, DECLEAD, DEC3D, DECTAGFONT, DECTALKFONT, DELTOPIC, UNKNOWN_CODE,
	DIMMUSIC, DROP, DROPEVERYTHING, DROPOUT, UNKNOWN_CODE, ENABLEMENU, ENDACTOR,
	ESCAPEOFF, ESCAPEON, EVENT, FACETAG, FADEIN, UNKNOWN_CODE, FADEOUT, FRAMEGRAB,
	FREEZECURSOR, GETINVLIMIT, GHOST, GLOBALVAR, UNKNOWN_CODE, HAILSCENE, HASRESTARTED, HAVE,
	UNKNOWN_CODE, HELDOBJECT, UNKNOWN_CODE, HIDEBLOCK, HIDEEFFECT,
	HIDEPATH, HIDEREFER, UNKNOWN_CODE, UNKNOWN_CODE, HOLD, HOOKSCENE, SETHYPERLINK,
	IDLETIME,
	INSTANTSCROLL, INVENTORY, INVPLAY, INWHICHINV, KILLACTOR,
	KILLGLOBALPROCESS, KILLPROCESS, LOCALVAR, MOVECURSOR, MOVETAG,
	MOVETAGTO, NEWSCENE, NOBLOCKING, NOPAUSE, NOSCROLL,
	UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE,
	OTHEROBJECT, PAUSE, OBJECTHELD, PLAY, PLAYMOVIE, PLAYMUSIC, PLAYSAMPLE,
	POINTACTOR, POINTTAG, POSTACTOR, UNKNOWN_CODE, POSTGLOBALPROCESS, POSTOBJECT,
	POSTPROCESS, POSTTAG, UNKNOWN_CODE, PRINT, PRINTCURSOR, PRINTOBJ, PRINTTAG,
	QUITGAME, RANDOM, RESETIDLETIME, RESTARTGAME, RESTORESCENE, RESUMELASTGAME, 
	RUNMODE, SAVESCENE, SAY, SAYAT, SCREENXPOS,
	SCREENYPOS, SCROLL, SCROLLPARAMETERS, SENDTAG, UNKNOWN_CODE, 
	UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE, UNKNOWN_CODE,
	SETINVLIMIT, SETINVSIZE, SETLANGUAGE, SETPALETTE, SETSYSTEMREEL,
	SETSYSTEMSTRING, SETSYSTEMVAR, UNKNOWN_CODE /*setview*/,
	SHELL, SHOWACTOR, SHOWBLOCK, SHOWEFFECT, SHOWMENU, SHOWPATH,
	UNKNOWN_CODE, UNKNOWN_CODE, SHOWTAG, STAND, STANDTAG,
	STARTGLOBALPROCESS, STARTPROCESS, STARTTIMER, UNKNOWN_CODE /*stopsamples*/,
	STOPSAMPLE, STOPWALK, SUBTITLES, SWALK, SWALKZ, SYSTEMVAR,
	TAGTAGXPOS, TAGTAGYPOS, TAGWALKXPOS, TAGWALKYPOS, TALK, TALKAT,
	TALKRGB, TALKVIA, TEMPTAGFONT, TEMPTALKFONT, THISOBJECT,
	THISTAG, TIMER, TOPIC, TOPPLAY, TOPWINDOW, UNDIMMUSIC,
	UNHOOKSCENE, WAITFRAME, WAITKEY, WAITSCROLL, WAITTIME,
	WALK, WALKED, UNKNOWN_CODE, UNKNOWN_CODE, WALKINGACTOR,
	WALKPOLY, WALKTAG, WALKXPOS, WALKYPOS, WHICHCD, WHICHINVENTORY,
	ZZZZZZ, LIB_STUB1, PLAYSEQUENCE, LIB_STUB2, LIB_STUB3
};
...
static void DecInvMain(CORO_PARAM, SCNHANDLE text, int MaxContents,
		int MinWidth, int MinHeight,
		int StartWidth, int StartHeight,
		int MaxWidth, int MaxHeight) {
	int idx;
	...
	idec_inv(1, text, MaxContents, 3, 2, 3, 2, 3, 2, 72, 39, false);
	idec_inv(3, 0, 3, 2, 2, 2, 1, 3, 1, 100, 100, false);
	idec_inv(4, text, MaxContents, 3, 2, 3, 2, 3, 2, 72, 39, false);

	idx = GetHandleIndex("english.scn");
	if(idx == -1)
		error("Could not find english.scn");
	
	HailScene(idx << SCNHANDLE_SHIFT);

	// This is done inside HailScene in Noir, but I figured this would be easier and have the same result.
	CORO_GIVE_WAY;
...
int CallLibraryRoutine(CORO_PARAM, int operand, int32 *pp, const INT_CONTEXT *pic, RESUME_STATE *pResumeState) {
	...
	else if(TinselV3) libCode = NOIR_CODES[operand];
	...
	case SETSYSTEMREEL:
		// Noir only
		pp -= 1;
		hSystemReels[pp[0]] = pp[1];
		return -2;

	case LIB_STUB1:
		warning("LIB_STUB1 libcall");
		return -8;

	case PLAYSEQUENCE:
		warning("PLAYSEQUENCE libcall");
		return -4;

	case LIB_STUB2:
		warning("LIB_STUB2 libcall");
		return -8;

	case LIB_STUB3:
		warning("LIB_STUB3 libcall");
		return -8;

	case DEC3D:
		warning("DEC3D libcall");
		return -3;

	case SETHYPERLINK:
		warning("SETHYPERLINK libcall");
		return -2;

	case DECINVMAIN:
		pp -= 7;
		DecInvMain(coroParam, pp[0], pp[1], pp[2], pp[3],
			pp[4], pp[5], pp[6], pp[7]);
		return -8;
	...
	case ADDINV4:
	case ADDINV3:
		//if(libCode == ADDINV8) AddInv(8, pp[0]);
		//else if(libCode == ADDINV7) AddInv(7, pp[0]);
		//else if(libCode == ADDINV4) AddInv(4, pp[0]);
		//else if(libCode == ADDINV3) AddInv(3, pp[0]);
		return -1;
	...
	case NOFUNCTION:
		error("NoFunction is not a real function");

	case NOIREMPTY:
		// Noir: no idea what this is. Just ignore it
		return -2;
