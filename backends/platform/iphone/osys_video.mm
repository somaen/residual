...
Graphics::PixelBuffer OSystem_IPHONE::setupScreen(int screenW, int screenH, bool fullscreen, bool accel3d) {
	//printf("initSize(%u, %u, %p)\n", width, height, (const void *)format);

	warning("setupScreen: %d %d %d %d", screenW, screenH, fullscreen, accel3d);

	// HACK:
	Graphics::PixelFormat *format = new Graphics::PixelFormat(2, 5, 6, 5, 0, 11, 5, 0, 0);

	_videoContext->screenWidth = screenW;
	_videoContext->screenHeight = screenH;
	...
	if (!format || format->bytesPerPixel == 1) {
		_framebuffer.create(screenW, screenH, Graphics::PixelFormat::createFormatCLUT8());
	} else {
#if 0
		...
		_framebuffer.w = screenW;
		_framebuffer.h = screenH;
	}
	...
	delete format;
	return Graphics::PixelBuffer(_framebuffer.format, (byte *)_framebuffer.pixels);
}
