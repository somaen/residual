...
#include "backends/platform/iphone/texture.h"
//#include "backends/platform/android/android.h"
//#include "backends/platform/android/jni.h"

#define printOpenGLError() printOglError3(__FILE__, __LINE__)

int printOglError3(const char *file, int line) {
	int retCode = 0;
...
void GLESBaseTexture::initGL() {
	...
	g_box_shader = Graphics::Shader::fromStrings("box", Graphics::BuiltinShaders::box_vertex, Graphics::BuiltinShaders::box_fragment, attributes);printOpenGLError();
	g_verticesVBO = Graphics::Shader::createBuffer(GL_ARRAY_BUFFER, sizeof(vertices), vertices);printOpenGLError();
	g_box_shader->enableVertexAttribute("position", g_verticesVBO, 2, GL_FLOAT, GL_TRUE, 2 * sizeof(float), 0);printOpenGLError();
	g_box_shader->enableVertexAttribute("texcoord", g_verticesVBO, 2, GL_FLOAT, GL_TRUE, 2 * sizeof(float), 0);printOpenGLError();
}
...
//	LOGD("*** Texture %p: Drawing %dx%d rect to (%d,%d)", this, w, h, x, y);
	...
	assert(g_box_shader);
	g_box_shader->use(); printOpenGLError();

	GLCALL(glBindTexture(GL_TEXTURE_2D, _texture_name)); printOpenGLError();
	const GLfloat offsetX    = float(x) / 768.0;//float(JNI::egl_surface_width);
	const GLfloat offsetY    = float(y) / 1024.0;//float(JNI::egl_surface_height);
	const GLfloat sizeW      = float(w) / 1024.0;//float(JNI::egl_surface_width);
	const GLfloat sizeH      = float(h) / 1024.0;//float(JNI::egl_surface_height);
...
//	LOGD("*** Drawing at (%f,%f) , size %f x %f", float(x) / float(_surface.w), float(y) / float(_surface.h),  tex_width, tex_height);
	...
	g_box_shader->setUniform("offsetXY", Math::Vector2d(offsetX, offsetY)); printOpenGLError();
	g_box_shader->setUniform("sizeWH", Math::Vector2d(sizeW, sizeH)); printOpenGLError();
	g_box_shader->setUniform("texcrop", Math::Vector2d(tex_width, tex_height)); printOpenGLError();
	g_box_shader->setUniform("flipY", !_is_game_texture); printOpenGLError();
...
void GLESTexture::drawTexture(GLshort x, GLshort y, GLshort w, GLshort h) {
	...
	if (!_dirty_rect.isEmpty()) {
		...
		GLCALL(glBindTexture(GL_TEXTURE_2D, _texture_name)); printOpenGLError();
		GLCALL(glPixelStorei(GL_UNPACK_ALIGNMENT, 1)); printOpenGLError();

		GLCALL(glTexSubImage2D(GL_TEXTURE_2D, 0,
								_dirty_rect.left, _dirty_rect.top,
								dwidth, dheight, _glFormat, _glType, _tex)); printOpenGLError();
	}
