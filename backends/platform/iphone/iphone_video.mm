...
- (void)createContext {
	...
	const char* vertexShaderSrc = "attribute vec2 position; attribute vec2 texcoord;varying vec2 v_texcoord;void main(){v_texcoord = texcoord; gl_Position = vec4(position,0,1);}";
	const char* fragmentShaderSrc = "precision mediump float;varying vec2 v_texcoord; uniform sampler2D sampler; void main() { gl_FragColor = texture2D(sampler, v_texcoord);}";
	...
	if (compileStatus) {
		...
		printf("It didn't compile");
		char errorBuffer[1024];
		errorBuffer[0] = '\0';
		glGetShaderInfoLog(vertexShader, 1024, 0, errorBuffer);
		printf("%s", errorBuffer);
	}
	GLuint fragShader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fragShader, 1, &fragmentShaderSrc, 0);
	...
	if (compileStatus) {
		...
		char errorBuffer[1024];
		errorBuffer[0] = '\0';
		glGetShaderInfoLog(fragShader, 1024, 0, errorBuffer);
		printf("%s", errorBuffer);
	}
	...
	if (programStatus) {
		...
		char errorBuffer[1024];
		errorBuffer[0] = '\0';
		glGetProgramInfoLog(program, 1024, 0, errorBuffer);
		printf("%s", errorBuffer);
	}
...
- (void)updateMainSurface {
	GLint shaderId = 0;
	glGetIntegerv(GL_CURRENT_PROGRAM, &shaderId); printOpenGLError();
	GLuint vertexPosStream = glGetAttribLocation(shaderId, "position"); printOpenGLError();
	glVertexAttribPointer(vertexPosStream, 2, GL_FLOAT, false, 0, _gameScreenVertCoords); printOpenGLError();
	glEnableVertexAttribArray(vertexPosStream); printOpenGLError();
	
	GLuint texCoordStream = glGetAttribLocation(shaderId, "texcoord"); printOpenGLError();
	glVertexAttribPointer(texCoordStream, 2, GL_FLOAT, false, 0, _gameScreenTexCoords); printOpenGLError();
	glEnableVertexAttribArray(texCoordStream); printOpenGLError();
...
- (void)updateOverlaySurface {
	...
	GLuint vertexPosStream = glGetAttribLocation(shaderId, "position");
	glVertexAttribPointer(vertexPosStream, 2, GL_FLOAT, false, 0, _gameScreenVertCoords);
	glEnableVertexAttribArray(vertexPosStream);
	
	GLuint texCoordStream = glGetAttribLocation(shaderId, "texcoord");
	glVertexAttribPointer(texCoordStream, 2, GL_FLOAT, false, 0, _gameScreenTexCoords);
	glEnableVertexAttribArray(texCoordStream);
