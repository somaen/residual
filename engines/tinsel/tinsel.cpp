 ...
 * $Id: tinsel.cpp 40890 2009-05-25 13:40:46Z lordhoto $
 *
 */
...
extern SCNHANDLE hSystemReels[41];
...
bool ChangeScene(bool bReset) {
	...
	if (NextScene.scene != 0) {
		if (!CountOut) {
			...
			if(TinselV3) {
				ClearScreen();
				if(!InMemory(NextScene.scene)) {
					if(!hSystemReels[31])
						return false;
					SCNHANDLE sysreel = hSystemReels[31];
					FILM *myfilm = (FILM *)LockMem(sysreel);
					MULTI_INIT *myinit = (MULTI_INIT *)LockMem(myfilm->reels[0].mobj);
					OBJECT* myobj = MultiInitObject(myinit);
					
					int whot = MultiRightmost(myobj);
				}
			}
...
void LoadBasicChunks(void) {
	...
	if(TinselV3) {
		noir_basic_info = FindChunk(MASTER_SCNHANDLE, CHUNK_BASIC_INFO);
		if(!noir_basic_info) 
			error("No game chunk has been found in the master scene file.");
		assert(READ_LE_UINT32(noir_basic_info) == 36); //Size of chunk
		assert(READ_LE_UINT32(noir_basic_info+4) == 4096); //No idea
		assert(!READ_LE_UINT32(noir_basic_info+8)); //Spacing perhaps
	}
	...
	cptr = TinselV3 ? noir_basic_info+12 : FindChunk(MASTER_SCNHANDLE, CHUNK_TOTAL_ACTORS);
	...
	cptr = TinselV3 ? noir_basic_info+16 : FindChunk(MASTER_SCNHANDLE, CHUNK_TOTAL_GLOBALS);
	RegisterGlobals((cptr != NULL) ? READ_LE_UINT32(cptr) : 512);

	cptr = TinselV3 ? noir_basic_info+32 : FindChunk(INV_OBJ_SCNHANDLE, CHUNK_TOTAL_OBJECTS);
	...
	cptr = TinselV3 ? noir_basic_info+20 : FindChunk(MASTER_SCNHANDLE, CHUNK_TOTAL_POLY);
	if (cptr != NULL)
		MaxPolygons(*cptr);

	if (TinselV2) {
		// Global processes
		cptr = TinselV3 ? noir_basic_info+24 : FindChunk(MASTER_SCNHANDLE, CHUNK_NUM_PROCESSES);
		...
		// CdPlay() stuff
		cptr = TinselV3 ? noir_basic_info+28 : FindChunk(MASTER_SCNHANDLE, CHUNK_CDPLAY_HANDLE);
		assert(cptr);
...
Common::Error TinselEngine::run() {
	// Initialize backend
	if (getGameID() == GID_DW2 || getGameID() == GID_NOIR) {
