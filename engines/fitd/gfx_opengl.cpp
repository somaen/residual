...
#include "engines/fitd/gfx_opengl.h"
...
//////// Callbacks we might not keep as callbacks ///////
#ifdef WIN32
...
void GfxOpenGL::init()  // that's the constructor of the system dependent
// object used for the SDL port
{
	...
	*/
	...
	// SDL_mixer init TODO make sound work
...
void GfxOpenGL::CopyBlockPhys(unsigned char *videoBuffer, int left, int top, int right, int bottom)
{
	...
	for(i=top;i<bottom;i++)
	{
		...
		for(j=left;j<right;j++)
		{
			...
			*(out++) = _palette[color*3];
			*(out++) = _palette[color*3+1];
			*(out++) = _palette[color*3+2];
		}
	}
...
void GfxOpenGL::flip(unsigned char *videoBuffer)
{
	int i;
	int j;
	int bestIdx;
	...
	for(j=0;j<positionInQuadTable;j++)
	{
		...
		glColor4ub(_palette[quadTable[bestIdx].color*3],_palette[quadTable[bestIdx].color*3+1],_palette[quadTable[bestIdx].color*3+2],quadTable[bestIdx].transparency);
...
void GfxOpenGL::fillPoly(float* buffer, int numPoint, unsigned char color,uint8 polyType)
{	
	switch(polyType)
	{
		case 0: // flat:
		...
		case 2: // trans
		{
			glColor4ub(_palette[color*3],_palette[color*3+1],_palette[color*3+2],128);
			glBegin(GL_POLYGON);
...
void GfxOpenGL::draw3dLine(float x1, float y1, float z1, float x2, float y2, float z2, unsigned char color)
{
	glColor4ub(_palette[color*3],_palette[color*3+1],_palette[color*3+2],255);
...
void GfxOpenGL::draw3dQuad(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3, float x4, float y4, float z4, unsigned char color, int transparency)
{
	glColor4ub(_palette[(color+3)*3],_palette[(color+3)*3+1],_palette[(color+3)*3+2],255);
	glBegin(GL_LINE_LOOP);
	glVertex3f(x1,y1,z1);
	glVertex3f(x2,y2,z2);
	glVertex3f(x3,y3,z3);
...
void GfxOpenGL::drawSphere(float X, float Y, float Z, uint8 color, float size)
{
	...
	glColor3ub(_palette[color*3],_palette[color*3+1],_palette[color*3+2]);
	glPushMatrix();
