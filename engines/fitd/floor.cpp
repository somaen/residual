...
void loadFloor(int floorNumber)
{
	...
	for(i=0;i<expectedNumberOfRoom;i++)
	{
		...
		warning("RoomDataTable: %d",(long)roomDataTable);
		if(g_fitd->getGameType() >= GType_AITD3)
		{
			char buffer[256];
		...
		{
			roomData = (unsigned char*) (etageVar0 +  READ_LE_UINT32(etageVar0 + i * 4));
		}
		warning("RoomDataTable: %d",(long)roomDataTable);
	...
	for(i=0;i<expectedNumberOfCamera;i++)
	{
		...
		if(offset<cameraDataSize)
		{
			...
			if(g_fitd->getGameType() < GType_AITD3)
			{
				currentCameraData = (unsigned char*) (etageVar1 + READ_LE_UINT32(etageVar1 + i * 4));
			}
	...
	warning("RoomDataTable final: %d", (long) roomDataTable);
